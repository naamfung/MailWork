...
import (
	...
	"MailWork/MailWork-Server/monkey"
	"MailWork/data"
	"MailWork/storage"
)
...
func RegisterFlags() {
	...
	flag.StringVar(&cfg.IMAPBindAddr, "imap-bind-addr", envconf.FromEnvP("MH_IMAP_BIND_ADDR", "0.0.0.0:143").(string), "IMAP bind interface and port, e.g. 0.0.0.0:143 or just :143")
	...
	flag.StringVar(&cfg.Hostname, "hostname", envconf.FromEnvP("MH_HOSTNAME", "mailwork.example").(string), "Hostname for EHLO/HELO response, e.g. mailwork.example")
	...
	flag.StringVar(&cfg.MongoDb, "mongo-db", envconf.FromEnvP("MH_MONGO_DB", "mailwork").(string), "MongoDB database, e.g. mailwork")
