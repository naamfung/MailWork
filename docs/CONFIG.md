Configuring MailWork
===================

You can configure MailWork using command line options or environment variables:
...
| MH_HOSTNAME         | -hostname       | mailwork.example | Hostname to use for EHLO/HELO and message IDs
...
| MH_IMAP_BIND_ADDR   | -imap-bind-addr | 0.0.0.0:143     | Interface and port for IMAP server to bind to
...
| MH_MONGO_DB         | -mongo-db       | mailwork         | MongoDB database name for message storage
...
| MH_UI_WEB_PATH      | -ui-web-path    |                 | WebPath under which the UI is served (without leading or trailing slashes), e.g. 'mailwork'
...
If you have MailWork behind a firewall, you'll need ports `8025` and `1025` by default.

You can override this using `-api-bind-addr`, `-ui-bind-addr` and `-smtp-bind-addr` configuration options.

If you're using MailWork behind a reverse proxy, e.g. nginx, make sure WebSocket connections
are also supported and configured - see [this issue](https://MailWork/issues/117) for information.
